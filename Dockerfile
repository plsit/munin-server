...
RUN set -xe \
    && apt-get update && RUNLEVEL=1 DEBIAN_FRONTEND=noninteractive && apt-get install -y \
        cron munin munin-node rrdcached \
        nginx apache2-utils \
        wget heirloom-mailx patch
...
RUN chmod +x /startrrd && chown -R munin:munin /var/log/munin /var/lib/munin /var/run/munin /var/cache/munin
